...
- session(1.将sessionId放在cookie里面;2.每一个请求都显示的传入一个sessionId的参数)
  - 服务器端
  - 安全
  - session可以依赖cookie，也可以不依赖使用url
  - 访问量增多，占用服务器资源，如果服务器挂了，所有保存的信息都没了
- token(需要程序员收到将token放在请求头中进行传递)
  - 服务器不存用户状态，定义通用算法
